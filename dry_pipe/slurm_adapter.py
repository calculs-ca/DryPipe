...
class SlurmArrayParentTask:
    ...
    def __init__(self, control_dir, tracker, task_conf, logger=None):
    ...
    def call_squeue_and_index_response(self, submitted_job_ids) -> dict[str,dict[int,str]]:

        res: dict[str,dict[int,str]] = {}
        for line in self.call_squeue(submitted_job_ids):
            line = line.strip()
    ...
    def compare_and_reconcile_squeue_with_state_files(self)-> dict[str, Tuple[str, str, str]]:

        dict_unexpected_states = {}

        submitted_job_ids: set[str] = self.submitted_job_ids()

        indexed_squeue_response: dict[str, dict[int,str]] = self.call_squeue_and_index_response(submitted_job_ids)
