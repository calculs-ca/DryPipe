...
class PipelineInstance:
    ...
    def regen_matching_tasks(self, task_key_glob_filter="*", log_handler=None, verbose=False):
        self._regen_tasks(task_key_glob_filter=task_key_glob_filter, cli_tail_logger=None, verbose=verbose)

    def regen_task(self, task_key, cli_tail_logger=None, verbose=False):
        self._regen_tasks(task_key_glob_filter=task_key, at_most_one=True, cli_tail_logger=cli_tail_logger, verbose=verbose)

    def _regen_tasks(self, task_key_glob_filter="*", at_most_one=False, cli_tail_logger=None, verbose=False):
        ...
        for task in state_machine.gen_all_tasks():
            if fnmatch.fnmatch(task.key, task_key_glob_filter):
                ...
                if verbose and cli_tail_logger is not None:
                    resolved_task = task.resolve()
                    s = resolved_task.summary()
                    cli_tail_logger.info(s)
                if at_most_one:
                    break
